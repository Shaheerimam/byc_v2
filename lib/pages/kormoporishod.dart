...
class kormo extends StatelessWidget {
  Future<List> _loaddata() async {
    ...
    var phone = c['phone'];
    var d =
        await FirebaseFirestore.instance.collection('list').doc('post').get();
    var post = d['post'];
    for (var i = 0; i < 7; i++) {
      var url = '';
      await FirebaseFirestore.instance
          .collection('users')
          .where('Post', isEqualTo: post[i])
          .limit(1)
          .get()
          .then((value) {
        url = (value.docs[0]['Url'].toString());
      }).then((value) {
        User1 user =
            User1(name: names[i], phone: phone[i], pic: url, post: post[i]);
        lis.add(user);
      });
    }
    return lis;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      //             ),
      //             IconButton(
      //                 onPressed: () => _makePhoneCall('tel:${phone[i]}'),
      //                 // onPressed: () => setState(() {
      //                 //       _launched =
      //                 //           _makePhoneCall('tel:${widget.phone[i]}');
      //                 //     }),
      //                 icon: Icon(
      //                   Icons.phone,
      //                   color: Colors.teal,
  ...
  FutureBuilder<List<dynamic>> mainpart3() {
                ...
                return ExpansionTile(
                  ...
                  leading: Container(
                      ...
                      decoration: BoxDecoration(
                          ...
                          image: new DecorationImage(
                              fit: BoxFit.cover,
                              image: new NetworkImage(snapshot.data[i].pic)))),
                  title: Text(
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      ...
                      children: [
                        Text(
                          "    যোগাযোগঃ ${snapshot.data[i].phone}",
                          textAlign: TextAlign.left,
                          style: TextStyle(color: Colors.teal, fontSize: 16),
                        ),
                        IconButton(
                            onPressed: () =>
                                _makePhoneCall('tel:${snapshot.data[i].phone}'),
                            // onPressed: () => setState(() {
                            //       _launched =
                            //           _makePhoneCall('tel:${widget.phone[i]}');
                            //     }),
